...
data = data.with_columns(
    ...
    pl.when(pl.col('VehAge') >= 20).then(pl.lit('20+')).otherwise(pl.col('VehAge')
                    .cast(pl.String).str.zfill(2)).cast(pl.Enum(
                        [str(s).zfill(2) for s in range(20)] + ['20+'])).alias("VehAge_New"),
...
# %% 
for (c, h, f, fm) in list(zip(cols, hues, filters, filters_as_string))[:1]:
    if f is None:
        temp = data
    else:
        temp = data.filter(f)
    ...
    fig, ax1 = plt.subplots(figsize=(12,8))
    sns.barplot(data = temp, x = c, y = 'Claim_Freq', hue=h,
                ax=ax1, legend=False)
    sns.lineplot(data = temp, x = c, y = 'Claim_Freq', hue=h,
                marker='o', sort = False, ax=ax1)
    sns.lineplot(data = temp, x = c, y = 'average_freq',
...
for (c, h, f, fm) in list(zip(cols, hues, filters, filters_as_string))[:1]:
    ...
    ax1.set(ylabel="Claim Frequency", title='Claim Frequency by {} and {}{}'.format(c, h, fm) )
# %%
# want to sort the categories/Enum values by the sum of exposure instead of the original order
sorting = temp.group_by(c).agg(pl.col("Total_Exposure").sum()).sort("Total_Exposure")
temp = temp.with_columns(pl.col("VehAge_New").cast(pl.Enum(sorting["VehAge_New"].to_list())))

fig, ax1 = plt.subplots(figsize=(12,8))
sns.barplot(data = temp, x = c, y = 'Claim_Freq', hue=h,
            ax=ax1, legend=False)
sns.lineplot(data = temp, x = c, y = 'Claim_Freq', hue=h,
            marker='o', sort = True, ax=ax1)
sns.lineplot(data = temp, x = c, y = 'average_freq',
            linestyle = '--', color = '#770737', sort = True, ax=ax1, 
            label="Average Claim Frequency")
...
g.legend(handles=[Rectangle((0,0), 0, 0, color='blue', label='Nontouch device counts'), 
                  Line2D([], [], marker='o', color='orange', label='Detections rate for nontouch devices')], 
         loc=(1.1,0.8))
# %%
#### CHANGE -inf and inf from cut/qcut?
my_min = str(data["BonusMalus"].min())
my_min
my_max = str(data["BonusMalus"].max())
my_max
enum_list = data["BonusMalus_Bands"].unique().cast(pl.String).str.replace_many(
    ["-inf", "inf"], [my_min,my_max]).to_list()
data["BonusMalus_Bands"].cast(pl.String).str.replace_many(
    ["-inf", "inf"], [my_min,my_max]).cast(pl.Enum(enum_list))
# %%
